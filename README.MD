...
It is built for Check Point Sales Engineers and PoC teams, and its SAML flow has been **validated end-to-end against four Check Point products** (see below).
...
The four integrations below have been tested through a full login round-trip. **Each ships pre-seeded as an editable template** (with placeholder identifiers) — on a fresh deploy you just replace the Entity ID / ACS URL with your environment's values; the attribute mappings are already filled in. Step-by-step recipes are in [SAML usage](#check-point-service-provider-recipes).
...
Each product below is pre-seeded as a template — replace the placeholder identifiers (`<your-mgmt-host>`, `<sp-id>`, `<your-tenant-id>`, `<your-gateway>`, `<region>`) with your values. Copy the Entity ID and ACS / Reply URL out of the Check Point product. The IdP signs both the Assertion and the Response, so all four accept it.
...
Full walkthrough (including the workaround for SASE's Entra-ID-only SCIM connector) is in [`docs/USER_GUIDE.md`](docs/USER_GUIDE.md). Short version:

1. **In Check Point SASE** — *Settings → Identity Providers → Add Microsoft Entra ID* (dummy credentials), enable **SCIM Integration**, generate a token, and copy the Tenant URL + token.
2. **In the simulator** — *Admin → SCIM → Outbound Targets → Add Target*, pick the region preset (US / EU / AU / IN), paste the token, save.
3. Click **Test**, then **Sync All** to push the demo users.

Every outbound request is recorded in **Admin → SCIM → Push Log** with full request/response bodies — handy for narrating a live demo.
